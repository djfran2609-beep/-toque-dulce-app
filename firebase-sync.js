...
(async function () {
  ...
  function mergeArrays(base, local, remote) {
    ...
    for (const k of order) {
      ...
      if (b && !l) {
        // Se eliminó localmente. Si el remoto no cambió, respetar eliminación.
        if (!r || same(r, b)) continue;
        // Si también cambió remotamente, conservar el remoto.
        out.push(clone(r));
        continue;
      }
  ...
  async function connectCloud(user) {
    ...
    try {
      ...
      if (first.exists() && first.data()?.state) {
        ...
        // Si este teléfono todavía tiene los datos de ejemplo/vacíos, usa la nube.
        // Si ya había datos reales locales, hace una combinación segura.
        const merged = merge3(remote, state, remote);
